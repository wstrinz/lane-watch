...
const upstream=Bun.serve({hostname:'127.0.0.1',port:0,idleTimeout:30,async fetch(req){
 ...
 return new Response(new ReadableStream({start(controller){controller.enqueue(new TextEncoder().encode(
  event('message_start',{message:{id:'msg_deadline_fixture',type:'message',role:'assistant',model:input.model,content:[],stop_reason:null,usage:{input_tokens:10,output_tokens:1}}})+event('ping')));}}),{headers:{'content-type':'text/event-stream'}});
}});
const gateway=Bun.serve({hostname:'127.0.0.1',port:0,idleTimeout:30,async fetch(req){
...
try{
 ...
 beforeCleanup=state;
 if(state?.state!=='stopped'||upstreamCalls!==1||errors.length)throw Error('Owned monitor did not stop the stalled native job at its deadline');
...
const report={at:new Date().toISOString(),fixture:'installed-native-background-deadline-gateway',preparedLeaseId:session,boundSession,jobId,failure,
...
writeFileSync(join(root,'report.json'),JSON.stringify(report,null,2));budget.close();console.log(JSON.stringify({report:join(root,'report.json'),...report}));if(failure||!monitorReady||monitorResult?.exit!==0||stop!==null||!monitorResult?.output?.includes('deadline'))process.exitCode=1;

...
const report={schema:'lane-watch-session-counter-binding/v1',observedAt:new Date().toISOString(),jobId:prior.jobId,sessionId:prior.sessionId,cliVersion:prior.cliVersion,stateSha256:prior.stateSha256,source:{sha256:source.observedFileSha256,bytes:source.observedFileBytes,originalAuditPrefixBytes:source.bytes,originalAuditPrefixSha256:source.sha256},record,totals,daemonReportedTokens:state.tokens,exactOutputMatch:totals.outputTokens===state.tokens,transcriptUniqueOutputTokens:transcriptOutputs,unreconciledOutputDifference:totals.outputTokens-transcriptOutputs,timeScope:{jobCreatedAt:state.createdAt,jobFirstTerminalAt:state.firstTerminalAt,sessionStartTime:new Date(record.startTime).toISOString(),sessionReportedDurationMilliseconds:record.totalDuration},elapsedSeconds:(performance.now()-started)/1000,limitations:['An exact numerical identity for this terminal job does not establish the adapter implementation or a supported metric across jobs.','The cost-state record was appended after the original four-transcript audit; its exact bytes and offset are bound separately.','The session summary contains more output units than deduplicated assistant messages in the four transcripts. No unobserved request, child, compaction or billing explanation is asserted.','The persisted session start predates the daemon job; no launch-time usage baseline was established by this audit.','Thinking fields are reported separately; their inclusion or overlap with output fields is not assumed.','No historical receipt/ledger, cap, epoch, runtime admission or mathematical authority changed.']};
const target=new URL('../runtime-audits/2026-09-05-bs21-session-counter-binding.json',import.meta.url);

...
function harness(values:Array<ResearchWatchObservation|null>,stopWorks=true,start=Date.parse(lease.startedAt)) {
 ...
 return {stops,events,port:{now:()=>now,monotonicNow:()=>now,observe:async()=>stopped?observation({state:'stopped',observedTokens:100}):values[Math.min(index++,values.length-1)],stop:async(id:string)=>{stops.push(id);stopped=stopWorks;},wait:async(ms:number)=>{now+=ms;},record:async(e:any)=>{events.push(e);}}};
}

...
function As(e) {
	...
	if (t?.status === "evidence_ready" && ["RESEARCH_READY", "RESEARCH_INTAKE"].includes(e.phase)) return {
		...
		detail: String(t.evidenceSummary?.verdict || t.evidence?.terminal_state || t.evidence?.verdict || t.evidenceSummary?.status || t.evidence?.status || "Receipt validated") + ". " + (e.researchPlan?.response?.lanes?.find((e) => e.taskId === t.taskId)?.question || t.taskId) + ". Continue to a synthesis of this result; no new research is launched.",
		actions: [{

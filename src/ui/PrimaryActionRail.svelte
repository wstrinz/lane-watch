<script lang="ts">
  ...
  function focusFor(value: any): RailFocus {
    ...
    if (value.phase === "RESEARCH_REVIEW") {
      ...
      if (blocked) return { status: "PLAN HELD", title: "Resolve the checked plan’s requirements", detail: compact(plan?.response?.quickChecks?.filter((check:any)=>check.status === "BLOCK").map((check:any)=>check.detail).join(" ") || plan?.response?.operatorGuidance || "The checked plan is held; inspect its requirements before requesting another review."), actions: [{ key: "inspect", label: "Read held plan and requirements", style: "primary-button" }] };
    ...
    <div class="rail-copy">
      ...
      <details class="rail-hint">
        ...
        <div>
          ...
          <p><b>Mathematical connection.</b> {compact((project.phase === "RESEARCH_REVIEW" ? project.researchPlan?.response?.lanes?.[0]?.evidenceExpected : project.researchSchedule?.members?.[0]?.expectedDelta) || project.researchPlan?.response?.lanes?.[0]?.evidenceExpected || direction || project.role, 420)}</p>
